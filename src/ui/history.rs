...
//! thread under the same single-flight discipline [`super::refresh`] uses for
//! the working-tree poll, so scrolling the git panel's History tab never
//! blocks on `git log`.
//!
...
//! [`super::App::poll_git_ops`]/[`super::App::poll_refresh`]), matching the
//! drained task's id against the single in-flight fetch so a foreign result
//! is dropped. Unlike the working-tree read, no generation guard is needed:
//! loaded pages are never invalidated, so no fetch can ever be superseded.
...
impl App {
    ...
    pub(super) fn request_history_page(&mut self) {
        ...
        if let Some(fetcher) = ops.async_commit_log_fetcher() {
            ...
            self.history_in_flight = Some(id);
    ...
    /// tick). Drops a foreign result (one whose id isn't the single in-flight
    /// fetch's) or a task-panic/git-error result silently; applies a
    /// successful page otherwise.
    pub(super) fn poll_history(&mut self) {
        for (id, result) in self.history_tasks.poll() {
            if self.history_in_flight != Some(id) {
                continue;
            }
