...
- [ ] 1.5 Refactor-only commit: replace `LauncherTab::toggle`'s two-tab assumption with ordered cycling over all variants (existing keys unchanged, last-used-tab memory intact); identical test counts before adding the third variant.
- [ ] 1.6 Add the **Pull Requests** tab: always-visible third variant; async list load through a new `stage_ops` fetcher seam + `BackgroundTasks` with single-flight and generation guard (mirror the Commits tab); shared `/` filter and motion layer; `Enter` stubbed to a status line until 2.0.
