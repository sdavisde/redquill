...
- FR-5: A new `Mode::ReviewLauncher` shall carry its tab, cursor, and an origin field. `Tab`/`l`/`Right`/`]` shall switch to the next tab and `Shift-Tab`/`h`/`Left`/`[` to the previous (directional split; bracket-key parity with other tabbed TUIs); `j`/`k` and arrow keys shall move the cursor; `Esc` shall close the modal and restore the invocation origin exactly — `Normal`/`Visual` returns to `Normal`; `Panel` returns to `Panel` with its cursor and tab intact (the `EndReviewOrigin` pattern).
